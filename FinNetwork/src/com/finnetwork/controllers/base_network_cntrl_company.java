...
public class base_network_cntrl_company {
	//	take the year parameter and return ObjectNode
	public ObjectNode get_base_network(String company) {
		...
		Query queryLink = session.createQuery("FROM Link WHERE fillingDate LIKE :company");
		queryLink.setParameter("company", "%"+company);		
		List<Link> links = queryLink.list();
		
		//	Select all the distinct source nodes from given year
		Query querySource = session.createQuery("SELECT DISTINCT source FROM Link WHERE fillingDate LIKE :company");
		querySource.setParameter("company", "%"+company);
		List<Integer> sources  = querySource.list();
		
		//	Select all the distinct target nodes from given year
		Query queryTarget = session.createQuery("SELECT DISTINCT target FROM Link WHERE fillingDate LIKE :company");
		queryTarget.setParameter("company", "%"+company);
